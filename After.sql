-- For Q3, Q7, the buffer pool size is increased, and the excution time decrease. I used the same query as BEFORE
-- Q3
...
-- rewrote query for Q18,Q23 after insert new column for month and year 
...
where u.subcategory = "GOP" and month_posts = 1 and year_posts = 2016 
...
where h.tid = t.tid and t.posts_by=u.screenname and u.subcategory =  "GOP" and year_posts= 2016 and find_in_set(month_posts, "1,2,3")
group by h.name
